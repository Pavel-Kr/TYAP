...
def parse_description(description):
    ...
    alphabet_in = split_set(desc[1])
    if alphabet_in is None:
        return None, "Синтаксическая ошибка в описании входного алфавита"
    dups = list(duplicates(alphabet_in))
    if len(dups) != 0:
        return None, f"Символы {dups} встречаются во входном алфавите больше одного раза"
    for a in alphabet_in:
        if len(a) != 1:
            return None, f"Неправильный символ входного алфавита {a}, длина символа должна быть равна 1"
    res["alphabet in"] = alphabet_in
    ...
    alphabet_out = split_set(desc[3])
    if alphabet_out is None:
        return None, "Синтаксическая ошибка в описании выходного алфавита"
    dups = list(duplicates(alphabet_out))
    if len(dups) != 0:
        return None, f"Символы {dups} встречаются в выходном алфавите больше одного раза"
    for a in alphabet_out:
        if len(a) != 1:
            return None, f"Неправильный символ выходного алфавита {a}, длина символа должна быть равна 1"
    res["alphabet out"] = alphabet_out
...
class DMP:
    ...
    def load_dmp_from_file(self, file_path):
        with open(file_path) as f:
            ...
            for i in range(len(table)):
                ...
                alphabet_in = self.dmp["alphabet in"]
                stack_alphabet = self.dmp["stack alphabet"]
                alphabet_out = self.dmp["alphabet out"]
                d = {}
                state = transitions[0]
                ...
                if state[1] != "" and state[1] not in alphabet_in:
                    return f"Синтаксическая ошибка в {i + 1}-м правиле: символ {state[1]} отсутствует в входном алфавите"
                ...
                for sym in action[2]:
                    if sym not in alphabet_out:
                        return f"Синтаксическая ошибка в {i + 1}-м правиле: символ {sym} отсутствует в выходном алфавите"
                state_tr = trans_dict.get(state[0])
                if state_tr is None:
                    d1 = {state[2]: action}#(action[0], action[1])}
                    d[state[1]] = d1
                ...
                if str_sym_tr is None:
                    d[state[2]] = action #(action[0], action[1])
                    state_tr[state[1]] = d
                    continue
                ...
                str_sym_tr[state[2]] = action #(action[0], action[1])
            self.dmp["transitions"] = trans_dict
            print(self.dmp)
            return ""

    def print_state(self):
        ret_input = self.input_string
        if ret_input == "":
            ret_input = "E"
        ret_stack = self.stack
        if ret_stack == "":
            ret_stack = "E"
        ret_output = self.output_string
        if ret_output == "":
            ret_output = "E"
        return f"({self.current_state}, {ret_input}, {ret_stack}, {ret_output})"
    ...
    def init_check(self, string):
        ...
        self.output_string = ""
        return True, self.print_state()

    def check_step(self):
        if self.input_string == "" and self.stack == "":
        ...
        if self.input_string != "":
            sym = self.input_string[0]
            if sym not in self.dmp["alphabet in"]:
                return False, f"\nЦепочка не распознана: символ {sym} отсутствует в входном алфавите"
        ...
        self.output_string = self.output_string + action[2]
        return True, self.print_state()
